...
bool less0(ElementPtr a, ElementPtr b) {
  return memcmp(a, b, 3) < 0;
}

bool test_dcslinear_lb() {
  bool pass = true;
  DcsLinear store = T2;
  char storedat[] = "ab0ab2ab4ab6ab8";
  memcpy(store.dat, storedat, store.itemsize * store.capacity);

  char *tin[] = {
    "aa0", "ab0", "ab1", "ab2", "ab8", "bb0"
  };
  DcsIterator expected[] = {
    0, 0, 1, 1, 4, 5
  };
  for (unsigned int i = 0; i<sizeof (tin) / sizeof (tin[0]); ++i) {
    DcsIterator result = _dcslinear_lower_bound(&store, less0, (ElementPtr) tin[i]);
    if (expected[i] != result) {
      fprintf(stderr, "_dcslinear_lower_bound() failed at test input #%u\n", i);
      pass = false;
    }
  }
  return pass;
